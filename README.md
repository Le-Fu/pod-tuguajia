...
- **macOS 10.12+**（当前提供 Intel x64 安装包，Apple Silicon 可通过 Rosetta 2 运行）
- 磁盘空间 ≥ 2GB（含本地 AI 模型）
...
| 5 | **抠图** | 智能去除图片背景 |
| 6 | **超分** | 低清图片放大增强（2/3/4 倍） |
| 7 | **压缩** | 批量把图片压到目标体积以下 |

> 💡 典型流程：**入库前去重 → 变形处理 → 超分/压缩 → 入库归档 → 图库浏览**
...
- 支持批量导入图片处理
- 模型在 **设置 → 模型配置 → 去背景模型** 中选择（本地 ONNX 离线可用，也可配云端接口）
- 处理后可结合「变形」模块换底、排版后导出
...
| **协作与连接** | 主从模式：同一局域网内多台设备可连接主机共享图库（mDNS 自动发现） |
| **数据优化** | 数据库维护、向量索引优化 |
